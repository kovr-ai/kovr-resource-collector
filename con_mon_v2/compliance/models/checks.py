    ...
    def evaluate(self, resources: List[Resource]) -> List["CheckResult"]:
        ...
        for resource in resources_to_check:
            try:
                ...
                actual_value = self._extract_field_value(resource, self.metadata.field_path)
                ...
                check_result = CheckResult(
                    ...
                    error=f"Field extraction failed: {str(field_error)}. Field path: {self.metadata.field_path} not found in resource {resource.id}"
                )
            ...
            try:
                ...
                passed = self.comparison_operation(actual_value, self.metadata.expected_value)
                check_result = CheckResult(
                    passed=passed,
                    check=self,
                    resource=resource,
                    message=f"Check '{self.name}' {'passed' if passed else 'failed'}. Expected: {self.metadata.expected_value}, Actual: {actual_value}"
                )
                ...
                check_result = CheckResult(
                    ...
                    error=f"Comparison failed: {str(compare_error)}. Field path: {self.metadata.field_path}, Expected: {self.metadata.expected_value}, Actual: {actual_value}"
                )
