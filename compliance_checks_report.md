# KOVR Resource Collector - Compliance Checks

This document tracks all implemented compliance checks for cloud resource monitoring and security assessment.

## Overview

# Checks for DOD SRG
- **Total Controls**: 0
- **Control Groups**: 0 control families
- **Group Coverage**: 0 of 0 families covered (100.0%)
- **Control Coverage**: 0 of 0 controls covered (0.0%)

## DOD SRG Control Family Coverage

| Family | Controls Covered | Total Controls | Coverage % | Description |
|--------|------------------|----------------|------------|-------------|

**Summary**: 0 of 0 families covered • 0 of 0 controls covered • 0 families above 10% coverage • 0 controls in uncovered families

# Checks for NIST 800-53
- **Total Controls**: 1,199
- **Control Groups**: 30 control families
- **Group Coverage**: 30 of 30 families covered (100.0%)
- **Control Coverage**: 0 of 1199 controls covered (0.0%)

## NIST 800-53 Control Family Coverage

| Family | Controls Covered | Total Controls | Coverage % | Description |
|--------|------------------|----------------|------------|-------------|
| Access Control | 0 | 147 | 0.0% | Access Control |
| Awareness and Training | 0 | 17 | 0.0% | Awareness and Training |
| Audit and Accountability | 0 | 69 | 0.0% | Audit and Accountability |
| Assessment, Authorization, and Monitoring | 0 | 32 | 0.0% | Assessment, Authorization, and Monitoring |
| Configuration Management | 0 | 66 | 0.0% | Configuration Management |
| Contingency Planning | 0 | 56 | 0.0% | Contingency Planning |
| Identification and Authentication | 0 | 70 | 0.0% | Identification and Authentication |
| Incident Response | 0 | 42 | 0.0% | Incident Response |
| Maintenance | 0 | 30 | 0.0% | Maintenance |
| Media Protection | 0 | 30 | 0.0% | Media Protection |
| Physical and Environmental Protection | 0 | 59 | 0.0% | Physical and Environmental Protection |
| Planning | 0 | 17 | 0.0% | Planning |
| Program Management | 0 | 37 | 0.0% | Program Management |
| Personnel Security | 0 | 18 | 0.0% | Personnel Security |
| PII Processing and Transparency | 0 | 21 | 0.0% | PII Processing and Transparency |
| Risk Assessment | 0 | 26 | 0.0% | Risk Assessment |
| System and Services Acquisition | 0 | 145 | 0.0% | System and Services Acquisition |
| System and Communications Protection | 0 | 162 | 0.0% | System and Communications Protection |
| System and Information Integrity | 0 | 118 | 0.0% | System and Information Integrity |
| Supply Chain Risk Management | 0 | 27 | 0.0% | Supply Chain Risk Management |
| DoD PKI authentication | 0 | 1 | 0.0% | DoD PKI authentication |
| DoD IP addressing | 0 | 1 | 0.0% | DoD IP addressing |
| Data Locations | 0 | 1 | 0.0% | Data Locations |
| Management Plane Connectivity | 0 | 1 | 0.0% | Management Plane Connectivity |
| CSO Personnel | 0 | 1 | 0.0% | CSO Personnel |
| Private Connection Availability Between CSPâ€™S/CSOâ€™s Network and DoD Network | 0 | 1 | 0.0% | Private Connection Availability Between CSPâ€™S/CSOâ€™s Network and DoD Network |
| Reliance on Internet | 0 | 1 | 0.0% | Reliance on Internet-Based Capabilities |
| Raliance of Internet Access | 0 | 1 | 0.0% | Raliance of Internet Access |
| CSP/CSOâ€™s Protection | 0 | 1 | 0.0% | CSP/CSOâ€™s Protection |
| Defense in depth architecture | 0 | 1 | 0.0% | Defense in depth architecture |

**Summary**: 30 of 30 families covered • 0 of 1199 controls covered • 0 families above 10% coverage • 1199 controls in uncovered families

# Checks for NIST 800-171 rev2 Catalog
- **Total Controls**: 110
- **Control Groups**: 14 control families
- **Group Coverage**: 14 of 14 families covered (100.0%)
- **Control Coverage**: 0 of 110 controls covered (0.0%)

## NIST 800-171 rev2 Catalog Control Family Coverage

| Family | Controls Covered | Total Controls | Coverage % | Description |
|--------|------------------|----------------|------------|-------------|
| Access Control | 0 | 22 | 0.0% | Access Control |
| Awareness and Training | 0 | 3 | 0.0% | Awareness and Training |
| Audit and Accountability | 0 | 9 | 0.0% | Audit and Accountability |
| Security Assessment | 0 | 4 | 0.0% | Security Assessment |
| Configuration Management | 0 | 9 | 0.0% | Configuration Management |
| Identification and Authentication | 0 | 11 | 0.0% | Identification and Authentication |
| Incident response | 0 | 3 | 0.0% | Incident response |
| Maintenance | 0 | 6 | 0.0% | Maintenance |
| Media Protection | 0 | 9 | 0.0% | Media Protection |
| Physical Protection | 0 | 6 | 0.0% | Physical Protection |
| Personnel Security | 0 | 2 | 0.0% | Personnel Security |
| Risk Assessment | 0 | 3 | 0.0% | Risk Assessment |
| System and Communications Protection | 0 | 16 | 0.0% | System and Communications Protection |
| System and Information Integrity | 0 | 7 | 0.0% | System and Information Integrity |

**Summary**: 14 of 14 families covered • 0 of 110 controls covered • 0 families above 10% coverage • 110 controls in uncovered families

# Checks for NIST CSF 2.0
- **Total Controls**: 106
- **Control Groups**: 16 control families
- **Group Coverage**: 16 of 16 families covered (100.0%)
- **Control Coverage**: 0 of 106 controls covered (0.0%)

## NIST CSF 2.0 Control Family Coverage

| Family | Controls Covered | Total Controls | Coverage % | Description |
|--------|------------------|----------------|------------|-------------|
| Govern | 0 | 31 | 0.0% | Govern |
| Asset Management | 0 | 8 | 0.0% | Asset Management |
| Risk Assessment | 0 | 10 | 0.0% | Risk Assessment |
| Improvement | 0 | 3 | 0.0% | Improvement |
| Identity Management | 0 | 6 | 0.0% | Identity Management |
| Awareness and Training | 0 | 2 | 0.0% | Awareness and Training |
| Data Security | 0 | 4 | 0.0% | Data Security |
| Platform Security | 0 | 6 | 0.0% | Platform Security |
| Technology Infrastructure Resilience | 0 | 4 | 0.0% | Technology Infrastructure Resilience |
| Continuous Monitoring | 0 | 5 | 0.0% | Continuous Monitoring |
| Adverse Event Analysis | 0 | 6 | 0.0% | Adverse Event Analysis |
| Incident Management | 0 | 5 | 0.0% | Incident Management |
| Incident Analysis | 0 | 4 | 0.0% | Incident Analysis |
| Incident Response Reporting and Communication | 0 | 2 | 0.0% | Incident Response Reporting and Communication |
| Incident Mitigation | 0 | 2 | 0.0% | Incident Mitigation |
| RECOVER | 0 | 8 | 0.0% | RECOVER |

**Summary**: 16 of 16 families covered • 0 of 106 controls covered • 0 families above 10% coverage • 106 controls in uncovered families

## Supported Frameworks

| Framework ID | Framework Name | Controls | Check Coverage | Description |
|-------------|----------------|----------|----------------|-------------|
| 1 | DOD SRG | 0 | 3303 checks | The Cloud Computing Security Requirements Guide (CC SRG) outlines the security model for DoD's use of cloud computing, detailing the necessary security controls and requirements for cloud-based solutions. |
| 2 | NIST 800-53 | 1,199 | 3303 checks | NIST cybersecurity framework is a powerful tool to organize and improve cybersecurity program |
| 3 | NIST 800-171 rev2 Catalog | 110 | 3303 checks | Security and Privacy Controls |
| 4 | NIST CSF 2.0 | 106 | 3303 checks | NIST CSF 2.0 cybersecurity framework |

## Resource Types

### AWS Resources (Connection ID: 1)
- **CloudTrailResource**: 1 fields - Trails, event selectors, insight selectors, tags
- **CloudWatchResource**: 4 fields - Log groups, metrics, alarms, dashboards
- **EC2Resource**: 8 fields - Instances, security groups, VPCs, networking
- **IAMResource**: 2 fields - Users, groups, roles, policies, access management
- **S3Resource**: 3 fields - Buckets, encryption, policies, lifecycle management

### GitHub Resources (Connection ID: 2)
- **GithubResource**: Repository-level security and configuration checks

---
*Report generated on 2025-08-11 16:46:42*