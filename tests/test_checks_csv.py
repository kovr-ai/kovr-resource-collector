...
def test_load_checks_from_csv():
    ...
    # Mock the database loading to prevent actual DB access
    with patch('con_mon_v2.checks.db.load_checks_from_database') as mock_db_load:
        mock_db_load.return_value = {}  # Return empty dict to prevent actual DB loading
        
        # Now we can safely import the module that auto-loads
